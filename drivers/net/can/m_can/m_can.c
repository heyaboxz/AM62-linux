...
static int m_can_open(struct net_device *dev)
{
	...
	if (cdev->is_peripheral) {
        ...
        /*
         * if request hardware interrupt failed(such as: mcu mcan)
         * register Implement High Resolution Timer interrupt (Linux kernel timer)
         * to replace hardware interrupt
         */
	if (err < 0) {
		dev_info(cdev->dev, "Enabling the hrtimer, irq request error code: %d\n", err);
		hrtimer_init(&cdev->hrtimer, CLOCK_MONOTONIC, HRTIMER_MODE_PINNED);
		cdev->hrtimer.function = &hrtimer_callback;
		hrtimer_start(&cdev->hrtimer, ns_to_ktime(0),
