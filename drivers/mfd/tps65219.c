			  ...
			  const struct i2c_device_id *ids)
{
	...
	sensor_mask = of_property_read_bool(tps->dev->of_node, "sensor-warm-mask");
	if (sensor_mask) {
		ret = regmap_read(tps->regmap, TPS65219_REG_MASK_CONFIG, &mask_config_reg);
		if (ret) {
			dev_err(tps->dev, "Failed to read mask config register: %d\n", ret);
			return ret;
		}

		/* turn off the temperature warning function */
		mask_config_reg |= 0xf;
		ret = regmap_write(tps->regmap, TPS65219_REG_MASK_CONFIG, mask_config_reg);
		if (ret) {
			dev_err(tps->dev, "Failed to write mask config register: %d\n", ret);
			return ret;
		}
	}
